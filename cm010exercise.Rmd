...
```{r}
time %>% 
  semi_join(album, by = c("song", "artist_name")) #filter and join == semi_join

# if want to display city, year, and album
time %>% 
  inner_join(album, by = c("song", "artist_name")) #mutate and join == semi_join
```
...
```{r}
time %>% 
  left_join(album, by = c("song", "artist_name")) #preserve NAs in original tibble 
```
